...
# ================== SIDEBAR ==================
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/2909/2909597.png", width=120)
...
def predict_trash(img: Image.Image):
    ...
    return labels[idx], float(output[idx] * 100)
...
video = st.file_uploader(
    "Quay video bỏ rác vào thùng (≥3 giây)",
    type=["mp4", "mov"]
)
...
if "used_videos" not in st.session_state:
    st.session_state.used_videos = set()
...
if video:
    ...
    if h in st.session_state.used_videos:
        st.error("❌ Video đã được sử dụng")
    elif size_mb < 1:
        st.error("❌ Video quá ngắn / không hợp lệ")
    else:
        st.success("🎥 Video hợp lệ – xác nhận hành vi")
        points_video = 10
        st.session_state.used_videos.add(h)
...
if points_video:
    points += 10
if points_image and points_video:
    points = 15
